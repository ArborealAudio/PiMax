...
public:
     ...
     * @tparam twoOutputs whether to generate two outputs from one input, or to process each input in place */
            ...
            for (size_t i = 0; i < low.getNumSamples(); ++i)
            {
                ...
                if (twoOutputs)
                {
                    for (size_t ch = 0; ch < low.getNumChannels(); ++ch)
                    {
                        ...
                        bands[0].processSample(ch, inL, inL, hi.getChannelPointer(ch)[i]);
                    }
                }
                else
                {
