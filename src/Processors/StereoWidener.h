...
template <typename T> class StereoWidener
{
  public:
    StereoWidener() {}
    ~StereoWidener() {}
    ...
    inline void widenBlockWithRamp(dsp::AudioBlock<T> &block, float beginWidth,
                                   float endWidth, bool isMono)
    {
        if (block.getNumChannels() < 2)
            return;
    ...
    inline void widenBufferWithRamp(AudioBuffer<T> &buffer, float beginWidth,
                                    float endWidth, bool isMono)
    {
        if (buffer.getNumChannels() < 2)
            return;
    ...
    void widenStereoSourceBlockWithRamp(dsp::AudioBlock<T> &block,
                                        float beginWidth, float endWidth)
    {
        auto xnL = block.getChannelPointer(0);
        auto xnR = block.getChannelPointer(1);
    ...
    void widenMonoSourceWithRamp(dsp::AudioBlock<T> &block, float beginWidth,
                                 float endWidth)
    {
        auto inc = (endWidth - beginWidth) / (float)block.getNumSamples();
    ...
    std::array<dsp::DelayLine<T, dsp::DelayLineInterpolationTypes::None>, 2>
        delay{{{dsp::DelayLine<T, dsp::DelayLineInterpolationTypes::None>(
                   44100.0)},
               {dsp::DelayLine<T, dsp::DelayLineInterpolationTypes::None>(
                   44100.0)}}};
};
