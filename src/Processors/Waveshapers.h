...
inline double gamma_PreAmp_v1(double xn, float gain)
{
    ...
    if (currentInputGain != lastInputGain)
            lastInputGain = 0.001f * currentInputGain + (1.0 - 0.001f) *
    lastInputGain; else lastInputGain = gain;*/
...
{
    // run twice for positive and negative, switching Ln and Lp, run again with
    // symmetric limits
    double yn = 0.0;
    if (xn <= 0)
        yn = (g * xn) / (1.0 - ((g * xn) / Ln));
...
inline double piShaper(double xn, float inputGain)
{
    ...
    yn = (std::atan(pow(pi, (e * inputGain * xn))) -
          0.5 / std::atan(pow(pi, (e * inputGain)))) /
         0.5;
...
inline double inflator(double xn, double k,
                       float clip) // includes phase inversion at high gain
{
    double yn = 0.0;
...
template <typename T>
void inflator_v2(T *xn, const size_t numSamples, T k, const ClipType clip,
                 const bool type) noexcept
{
    if (type)
    ...
    case ClipType::Deep: {
        ...
        for (int i = 0; i < numSamples; ++i)
            xn[i] = xn[i] / std::pow((1.0 + std::pow(std::abs(xn[i]), curve)),
                                     1.0 / curve);
    ...
    default:
        for (size_t i = 0; i < numSamples; ++i) {
            ...
            if (x > -1.0 && x < 1.0) {
                if (k > 1.0) {
                    yn = dsp::FastMathApproximations::sin(
                        halfPi * pow(k, acos(x * x)) * x);
                } else {
                    auto nk = 1.0 / k;
                    yn = dsp::FastMathApproximations::sin(
                        halfPi * pow(nk, log10(x * x)) * x);
                }
            }
